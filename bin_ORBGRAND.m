...
function [y_decoded,err_vec,n_guesses,abandoned,APP] = bin_ORBGRAND(H,max_query,y_soft,even_code)
    ...
    while n_guesses<max_query
        ...
        while w<=floor((sqrt(1+8*W)-1)/2)
            ...
            for jj=1:size(noise_locations,1)
                    ...
                    err_vec(indexes)=1;
                    pG = pG1*prod(p_demod(indexes)./(1-p_demod(indexes)));
                    sPG = sPG+pG;
