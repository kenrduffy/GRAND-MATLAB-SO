...
%
% Edited by Amit Solomon to support vectors with keys. Some of the functions (that are not used) may not work
%--------------------------------------------------------------------------
...
%             this.x(i) = key;
            this.x{i} = key;
%             while ((i > 1) && (this.x(Heap.parent(i)) < this.x(i).score))
            while ((i > 1) && (this.x{Heap.parent(i)}.score < this.x{i}.score))
...
%             if ((ll <= size) && (this.x(ll) > this.x(i)))
            if ((ll <= size) && (this.x{ll}.score > this.x{i}.score))
                largest = ll;
            else
                largest = i;
            end
%             if ((rr <= size) && (this.x(rr) > this.x(largest)))
            if ((rr <= size) && (this.x{rr}.score > this.x{largest}.score))
                largest = rr;
            end
            if (largest ~= i)
                this.Swap(i,largest);
